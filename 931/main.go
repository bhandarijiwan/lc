...
func minPathSum(A [][]int, result [][]int, row, col int) int {
	m := len(A)
	if row >= m {
		return 0
	}
	...
	v := A[row][col]
	left := minPathSum(A, result, row+1, col-1)
	center := minPathSum(A, result, row+1, col)
	right := minPathSum(A, result, row+1, col+1)
	r := min(left, center, right) + v
	result[row][col] = r
	return r
}
...
func minFallingPathSum(A [][]int) int {
	...
	for i := l - 2; i >= 0; i-- {
		for j := 0; j < l; j++ {
			...
			r[i][j] = min(left, right, center) + A[i][j]
		}
	}
	return min(r[0]...)
}
...
func main() {
	...
	fmt.Println(minFallingPathSum(input))
}
